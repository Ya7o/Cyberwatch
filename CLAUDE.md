...
- **Pas de donnée inventée** : absence de preuve, panne réseau, budget ou clé manquante -> `Inconnu` ou statut explicite.
  ...
  - **Architecture Sector (refonte 2026-08-26, "preuves partout, décision unique à la fin")** : seules deux autorités court-circuitent tout le reste sans jamais appeler de LLM — une validation humaine (`data/enrichment_reference.csv`) ou un code NAF officiel (`Validated_Via == "deterministic"` dans `org_enrichment_cache.csv`, jamais un texte de site officiel scrappé). Tout le reste (mots-clés déterministes, texte d'article, site officiel scrappé, secteur structuré d'une source...) n'est plus jamais appliqué directement à `Item.Sector` — chaque mécanisme ne fait plus que déposer une preuve, consommée par `organisation_sector.py`. Pour toute organisation qu'aucune des deux autorités n'a résolue, `organisation_sector_llm.py` devient une étape **obligatoire** de `qualify()` (plus une commande manuelle à part) : un appel LLM automatique par organisation à chaque `create`/`maj`, qui voit l'ensemble des preuves faibles déjà collectées et rend la décision finale. Non bloquant (budget épuisé ou erreur API -> `Inconnu`, jamais un échec de run), mais un coût LLM désormais systématique pour toute organisation sans NAF/référence — à surveiller via le canal `organisation_sector` du rapport budget de `cold-reset.yml`.
