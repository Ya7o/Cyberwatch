...
def test_decisions_from_provenance_preserves_rejection():
    rows = [
        {
            ...
            "Previous_Value": config.SECTOR_UNKNOWN,
            "Candidate_Value": config.SECTOR_HEALTH,
            "Final_Value": config.SECTOR_UNKNOWN,
    ...
    assert decision.origin == "LLM_SOURCE_FALLBACK"
...
def test_summary_groups_by_origin_and_field():
    decisions = [
        QualificationDecision(
            "ITEM-1", "A", "Sector", "Inconnu", "Santé", "Santé",
            "MANUAL_REFERENCE", "HIGH",
        ),
        QualificationDecision(
            "ITEM-2", "A", "Sector", "Inconnu", "Industrie", "Inconnu",
            "LLM_SOURCE_FALLBACK", "", decision="REJECTED_POLICY_DISABLED",
        ),
        QualificationDecision(
            "ITEM-3", "A", "Threat", "Inconnu", "Ransomware", "Inconnu",
            "LLM_SOURCE_FALLBACK", "", decision="PROTECTED",
        ),
    ]

    summary = summarize_decisions(decisions)
    by_key = {(row["Origin"], row["Field"]): row for row in summary}
