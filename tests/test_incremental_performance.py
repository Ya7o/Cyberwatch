...
def test_frenchbreaches_detail_cache_avoids_second_network_fetch(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("FRENCHBREACHES_DETAIL_CACHE_PATH", str(cache_path))
    monkeypatch.setenv("FRENCHBREACHES_DETAIL_CACHE_TTL_DAYS", "7")
    incremental_performance.reset_for_tests()
    ...
    class Response:
        ok = True
        text = "<article>Texte détaillé stable</article>"
    ...
    client = Client()
    first = _entry(content="")
    attempted, hydrated = feed._hydrate_frenchbreaches_details(client, [first], Budget())
    assert (attempted, hydrated) == (1, 1)
    assert client.calls == 1
    assert first.content

    second = _entry(content="")
    attempted, hydrated = feed._hydrate_frenchbreaches_details(client, [second], Budget())
    assert (attempted, hydrated) == (1, 1)
    assert client.calls == 1
...
def test_frenchbreaches_detail_cache_invalidates_when_feed_identity_changes(monkeypatch, tmp_path):
    ...
    client = Client()
    feed._hydrate_frenchbreaches_details(client, [_entry(content="")], Budget())
    changed = _entry(content="")
    changed.summary = "Résumé modifié"
    feed._hydrate_frenchbreaches_details(client, [changed], Budget())
    assert client.calls == 2


def test_performance_history_is_bounded_and_replaces_same_run(monkeypatch, tmp_path):
    path = tmp_path / "performance.json"
    monkeypatch.setenv("CYBERWATCH_PERFORMANCE_LOG_PATH", str(path))
    incremental_performance._save_performance_row({"run_id": "R1", "duration_s": 10})
    incremental_performance._save_performance_row({"run_id": "R1", "duration_s": 8})
    rows = incremental_performance._load_performance_history()
    assert rows == [{"run_id": "R1", "duration_s": 8}]
