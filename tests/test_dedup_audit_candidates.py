...
def test_shared_company_id_surfaces_unresolved_identity(make_item):
    ...
    company_ids = {left.Organisation_Key: "123456789", right.Organisation_Key: "123456789"}
...
def test_ransomware_j4_merge_is_exposed_for_review(make_item):
    left = make_item(
        source="RANSOMWARE_LIVE", org="FILAIR", published="2026-04-13",
        threat="Ransomware", title="FILAIR revendiqué", url="https://a",
    )
    right = make_item(
        source="CYBERATTAQUE_ORG", org="Filair", published="2026-04-17",
        threat="Ransomware", title="Filair : archives publiées après cyberattaque", url="https://b",
    )
    candidates = find_audit_candidates([left, right])
    assert len(candidates) == 1
    assert candidates[0].risk_type == RISK_FALSE_MERGE
    assert candidates[0].reason_code == MERGE_REVIEW_RANSOMWARE_CORROBORATION
