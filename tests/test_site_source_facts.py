...
def _rejected_sector_row(item_id: str, candidate: str, decision: str = "REJECTED_IDENTITY_EVIDENCE") -> dict:
    return {
        "Item_ID": item_id,
        "Field": "Sector",
...
def test_secteur_suppose_expose_pour_un_candidat_unique_non_confirme():
    items = [_item("ITM-a", "FRENCHBREACHES")]
    rows = [_rejected_sector_row("ITM-a", "Santé")]

    payload = site._qualification_provenance_by_incident(items, rows)

    assert len(payload) == 1
    assert next(iter(payload.values())) == "Santé"


def test_secteur_suppose_absent_sans_aucun_candidat():
    items = [_item("ITM-a", "FRENCHBREACHES")]

    assert site._qualification_provenance_by_incident(items, []) == {}


def test_secteur_suppose_absent_si_deux_items_du_meme_incident_se_contredisent():
    items = [
        _item("ITM-a", "FRENCHBREACHES"),
        _item("ITM-b", "CYBERATTAQUE_ORG"),
    ]
    rows = [
        _rejected_sector_row("ITM-a", "Santé"),
        _rejected_sector_row("ITM-b", "Finance / Assurance"),
    ]

    assert site._qualification_provenance_by_incident(items, rows) == {}


def test_secteur_suppose_absent_si_un_seul_item_a_des_candidats_contradictoires():
    items = [_item("ITM-a", "FRENCHBREACHES")]
    rows = [
        _rejected_sector_row("ITM-a", "Santé"),
        _rejected_sector_row("ITM-a", "Finance / Assurance"),
    ]

    assert site._qualification_provenance_by_incident(items, rows) == {}


def test_secteur_suppose_ignore_un_conflit_interne_meme_a_un_seul_exemplaire():
    items = [_item("ITM-a", "FRENCHBREACHES")]
    rows = [_rejected_sector_row("ITM-a", "Santé", decision="REJECTED_SECTOR_CONFLICT")]

    assert site._qualification_provenance_by_incident(items, rows) == {}


def test_secteur_suppose_ignore_une_raison_de_rejet_inconnue():
    items = [_item("ITM-a", "FRENCHBREACHES")]
    rows = [_rejected_sector_row("ITM-a", "Santé", decision="REJECTED_UN_MOTIF_FUTUR")]

    assert site._qualification_provenance_by_incident(items, rows) == {}


def test_secteur_suppose_atteint_le_payload_quand_le_secteur_canonique_est_inconnu():
    item = _item("ITM-a", "FRENCHBREACHES")
    incident_id = identity.incident_id(item.Organisation_Key, item.Item_ID)
    incident = _incident(incident_id)
    incident.Secteur = config.SECTOR_UNKNOWN
    tentative = {incident_id: "Santé"}

    row = site.incidents_payload([incident], tentative_sectors=tentative)[0]

    assert row["sector"] == config.SECTOR_UNKNOWN
    assert row["sector_tentative"] == "Santé"


def test_secteur_suppose_jamais_ajoute_si_le_secteur_canonique_est_deja_resolu():
    item = _item("ITM-a", "FRENCHBREACHES")
    incident_id = identity.incident_id(item.Organisation_Key, item.Item_ID)
    incident = _incident(incident_id)
    assert incident.Secteur != config.SECTOR_UNKNOWN
    # Provenance obsolète : ne doit jamais entrer en conflit avec un secteur
    # déjà résolu, même si elle traîne encore dans le fichier.
    tentative = {incident_id: "Santé"}

    row = site.incidents_payload([incident], tentative_sectors=tentative)[0]

    assert "sector_tentative" not in row


def test_build_charge_explicitement_qualification_provenance():
    source = open("cyberwatch/site.py", encoding="utf-8").read()
