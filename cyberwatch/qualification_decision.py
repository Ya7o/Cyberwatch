...
def record_mutations(before, items, *, origin: str, confidence: str, evidence: str = "", match_strategy: str = ""):
    decisions = []
    for item in items:
        ...
        for field in QUALIFICATION_FIELDS:
            ...
            if old != new:
                decisions.append(QualificationDecision(item.Item_ID, item.Source_ID, field, old, new, new,
                                                       origin, confidence, evidence, match_strategy, "APPLIED"))
    return sorted(decisions, key=_decision_sort_key)

def decisions_from_provenance(rows):
    return sorted((QualificationDecision.from_provenance(row) for row in rows), key=_decision_sort_key)
...
def summarize_decisions(decisions):
    ...
    for (origin, field), values in sorted(grouped.items()):
        ...
        rows.append({"Origin": origin, "Field": field, "Decisions": len(values),
                     "Applied": statuses.get("APPLIED", 0),
                     "Rejected": sum(v for k, v in statuses.items() if k.startswith("REJECTED")),
                     "Protected": statuses.get("PROTECTED", 0),
                     "Other": sum(v for k, v in statuses.items() if k not in {"APPLIED", "PROTECTED"} and not k.startswith("REJECTED")),
                     "Confidence": dict(sorted(confidences.items()))})
    return rows

def precedence(origin: str) -> int:
    """Rang contractuel d'une origine; plus petit = plus autoritaire."""
    return ORIGIN_PRIORITY.get(origin, 999)

def _decision_sort_key(decision):
    return (decision.item_id, decision.field, precedence(decision.origin), decision.origin, decision.decision)
