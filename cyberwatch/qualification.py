...
def backfill_structured_source_sectors(items: list[Item], source_fact_rows=None) -> int:
...
def neutralize_sector_fallback(items, changes, provenance):
    ...
    for row in provenance:
        ...
        row["Final_Value"], row["Confidence"], row["Decision"] = item.Sector, "", "REJECTED_POLICY_DISABLED"; neutralized += 1
    if neutralized:
...
def qualify(items: list[Item]) -> QualificationReport:
    ...
    changes = _observe_layer(decisions, ordered, origin="MANUAL_REFERENCE", confidence="HIGH", mutate=lambda: enrichment.enrich_items(ordered, reference))
    ...
    registry_rows = sector_registry.build_registry(ordered, reference, source_fact_rows=source_facts, org_cache_rows=org_cache, previous_provenance=previous_provenance)
    ...
    decisions.extend(decisions_from_provenance(registry_provenance)); changes["sector_registry_applied"], changes["sector_registry_known_conflicts"] = registry_applied, registry_known_conflicts
