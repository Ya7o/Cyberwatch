...
Les producteurs historiques peuvent encore calculer leurs candidats comme avant,
mais la valeur finale et l'explication des perdants sont centralisées ici.
"""
...
def choose_winner(candidates: list[QualificationCandidate]) -> QualificationCandidate | None:
    ...
    return min(usable, key=lambda candidate: (
        precedence(candidate.origin), candidate.origin, candidate.value,
        candidate.match_strategy, candidate.evidence,
    ))


def reconcile(items: list[Item], decisions: list[QualificationDecision]) -> list[QualificationDecision]:
    """Applique un arbitrage final déterministe et explique tous les candidats perdants.

    Seules les décisions ayant réellement été appliquées sont candidates au changement
    de valeur. Les décisions déjà rejetées/protégées restent auditables et reçoivent,
    lorsqu'il existe, l'origine gagnante du même item/champ.
    """
    by_key: dict[tuple[str, str], list[QualificationDecision]] = defaultdict(list)
    for decision in decisions:
        by_key[(decision.item_id, decision.field)].append(decision)
    items_by_id = {item.Item_ID: item for item in items}
    output: list[QualificationDecision] = []
    for key, rows in sorted(by_key.items()):
        ...
        for row in rows:
            is_winner = winner is not None and row.origin == winner.origin and (row.candidate_value or row.final_value) == winner.value and row.decision == "APPLIED"
            if is_winner:
                ...
                continue
            if row.decision == "APPLIED" and winner is not None:
            ...
            output.append(replace(row, final_value=winning_value or row.final_value,
                                  rejected_reason=reason, winning_origin=winning_origin, winning_value=winning_value))
