...
def classify_items(items: Iterable[Item], previous_fingerprints: Mapping[str, str], *, facts_by_item: Mapping[str, Iterable[Mapping[str, object]]] | None = None, policy_version: str, dependency_digest_value: str = "") -> DirtySet:
    ...
    for item in sorted(items, key=lambda value: value.Item_ID):
        fingerprint = qualification_fingerprint(item, facts_by_item.get(item.Item_ID, ()), policy_version=policy_version, dependency_digest_value=dependency_digest_value)
...
def fingerprints_from_state(rows: Iterable[Mapping[str, object]]) -> dict[str, str]:
    return {str(row.get("Item_ID") or ""): str(row.get("Qualification_Fingerprint") or "") for row in rows if row.get("Item_ID") and row.get("Qualification_Fingerprint")}
