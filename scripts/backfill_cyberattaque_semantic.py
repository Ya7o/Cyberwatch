...
def run(*, endpoint: str, start: str, max_calls: int, http_timeout: int, progress_path: Path) -> dict:
    ...
    try:
        # Premier passage déterministe : aucune dépense LLM, mais permet de savoir
        # quels articles sont réellement candidats selon la même politique.
        ...
        for post in posts:
            ...
            # should_use_llm dépend aussi du flag d'activation ; on reproduit ici
            # ses critères après avoir construit la base déterministe.
        ...
        # Les cache hits sont gratuits et sont appliqués même lorsque le budget
        # de nouveaux appels est nul. Les misses sont bornés explicitement.
        os.environ["CYBERATTAQUE_SEMANTIC_ENABLED"] = "1"
        ...
        for item, text, post in backlog:
            ...
            if not before:
                if key in after_cache:
                    stats["llm_calls"] += 1
                    calls_left -= 1
                else:
                    # L'appel peut avoir échoué/abstenu techniquement : il reste
                    # dans le backlog et ne doit pas être compté comme accompli.
                    stats["failed"] += 1
                    stats["backlog_remaining"] += 1
                    calls_left -= 1
                    continue
